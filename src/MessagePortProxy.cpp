...
int
MessagePortProxy::UnRegisterMessagePort(const string& localPort, bool isTrusted)
{
	_SECURE_LOGI("UnRegister a message port : [%s:%s]", __appId.c_str(), localPort.c_str());
	...
	IPC::Message* pMsg = new MessagePort_unregisterPort(buffer, &return_value);
	if (pMsg == NULL)
	{
		_LOGE("Out of memory");
		return  MESSAGEPORT_ERROR_OUT_OF_MEMORY;
	...
	// remove listener
	if (!isTrusted)	{
		__listeners.erase(localPort);
	}
	else {
		__trustedListeners.erase(localPort);
	}
