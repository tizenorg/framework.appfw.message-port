...
static gboolean unregistermessageport(gpointer key, gpointer value, gpointer user_data)
{
	...
	int *removeclient = (int *)user_data;

	if (*clientId == *removeclient)
	{
		return true;
	}
...
int
_MessagePortService::UnRegisterMessagePort(int clientId, const BundleBuffer& buffer)
{
	...
	_LOGI("UnRegister a message port: [%s], client = %d", key, clientId);
	...
	*id = clientId;
	...
	if (strncmp(trusted, "TRUE", 4) == 0)
	{
		...
		if (value)
		{
			_SECURE_LOGI("Remove from trustedPorts : %s", key);
			g_hash_table_remove(__pTrustedPorts, key);
		}
	}
	else
	{
		...
		if (value)
		{
			_SECURE_LOGI("Remove from ports : %s", key);
			g_hash_table_remove(__pPorts, key);
		}
	}
