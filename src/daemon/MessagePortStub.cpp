...
bool
_MessagePortStub::OnUnRegisterMessagePort(const BundleBuffer& buffer, int* pResult)
{

	_LOGI("_MessagePortStub::OnUnRegisterMessagePort.");
	assert(__pIpcServer);

	int clientId = __pIpcServer->GetClientId();
	*pResult = __pService->UnRegisterMessagePort(clientId, buffer);
	bundle_free(buffer.b);
	return true;
}
...
_MessagePortStub::OnIpcRequestReceived(_MessagePortIpcServer& server, const IPC::Message& message)
{
	...
	IPC_BEGIN_MESSAGE_MAP(_MessagePortStub, message)
		...
		IPC_MESSAGE_HANDLER_EX(MessagePort_unregisterPort, &server, OnUnRegisterMessagePort)
